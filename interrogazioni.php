...
if ($_GET["scope"] === "getAllData") {
    ...
    header('Content-Type: application/json');
    die(json_encode(array("status" => $okay, "newData" => ($okay ? array("subjects" => getAllData(), "users" => json_decode(file_get_contents("./JSON{$PROFILE}/users.json"), true), "profiles" => ($PROFILE === "" ? $profileList : false), "currentProfile" => $PROFILE) : false))));
